...
if ($dbSuccess) {
	...
	//	$fld_Salutation = '<input type="text" name="Salutation" value="'.$current_Salutation.'"/>';
		...
		{	//	create the Company DROPDOWN  FIELD 
			...
			$companyList_SQL_Query = mysqli_query($dbConnected,$companyList_SQL);	

			$fld_companyID = '<select name="companyID">';
				//$fld_Salutation .= '<option selected="selected"></option>';
	 	
				while ($row = mysqli_fetch_array($companyList_SQL_Query, MYSQLI_ASSOC)) {
				    $dd_companyID = $row['ID'];
				...
				mysqli_free_result($companyList_SQL_Query);		
